 ...
 *
 * Uses estimateAvailableSlots() throughout — it exercises the same resolveOpenWindow() precedence
 * logic as the full solver but runs in milliseconds without invoking CP-SAT.
 *
 * Key dates used across these tests:
 *   June 1, 2026  = Monday
...
class SchedulerServiceDowTest {
    ...
    /**
     * Slot count for a window [open, close) with 60-minute games on a 15-minute grid:
     *   count = (close - open) / 15 min   (the last start time is at close - 60)
     *   e.g. 09:00–18:00 → (9h * 60 / 15) = 36, but last start is 17:00, so 33 slots.
    ...
    class BlockedDays {
        ...
        void blockedDayIsOmittedFromMultiDaySeason() {
            ...
            assertEquals(slotsWithoutBlock - perDaySlots, slotsWithBlock,
    ...
    class OverrideBeatsBock {
        ...
        void overrideOnBlockedDayReturnsSlots() {
            ...
            FieldDateOverride override = new FieldDateOverride(
                UUID.randomUUID(), SUNDAY, T9, T13);  // 09:00–13:00
            ...
            assertEquals(expected, slots,
                "FieldDateOverride should rescue slots on a blocked day");
        }
    ...
    class DowWindows {
        ...
        void dowWindowNarrowsSlots() {
            ...
            assertEquals(expected, slots,
                "slot count should match the DOW window, not the global sunrise/sunset");
        }
        ...
        void dowWindowForOtherDayDoesNotApply() {
            ...
            assertEquals(expected, slots,
                "Wednesday DOW window should not affect a Monday-only season");
        }
        ...
        void dowWindowAppliesConsistentlyAcrossAllMatchingDays() {
            ...
            int slotsWithoutWindow = service.estimateAvailableSlots(leagueNoWindow, divId(leagueNoWindow), GAME_DURATION);
            ...
            assertEquals(slotsWithoutWindow - diff, slotsWithWindow,
    ...
    class Precedence {
        ...
        void overrideBeatsDowWindow() {
            ...
            int expectedFromOverride = expectedSlots(T9, T13, GAME_DURATION);
            assertEquals(expectedFromOverride, slots,
                "FieldDateOverride should win over a DOW window");
        }
