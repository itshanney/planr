...
class TeamScheduleServiceTest {
    ...
    private static final LeagueConfig VALID_CONFIG = new LeagueConfig(
        ...
        LocalDate.of(2026, 6, 1), LocalDate.of(2026, 8, 31), List.of(), List.of(), null, null);
    ...
    @Test
    @DisplayName("3 teams (odd count) with target=2 produces 3 round-robin games with no bye teams in output")
    void threeTeamsProducesThreeGamesWithNoByeTeams() {
        ...
        for (UUID id : List.of(a.id(), b.id(), c.id())) {
            assertEquals(2, gamesPlayedBy(games, id),
    ...
    void fourTeamsProducesSixRoundRobinGames() {
        ...
        for (UUID id : List.of(a.id(), b.id(), c.id(), d.id())) {
            assertEquals(3, gamesPlayedBy(games, id),
    ...
    void fourTeamsRoundRobinHomeAwayBalanced() {
        ...
        for (Team t : List.of(a, b, c, d)) {
            ...
            assertTrue(Math.abs(home - away) <= n - 1,
                "Team " + t.name() + " home/away imbalance > " + (n - 1) + ": home=" + home + " away=" + away);
        }
    }
    ...
    void fourTeamsFillProducesSixteenGames() {
        ...
        for (Team t : List.of(a, b, c, d)) {
            assertEquals(8, gamesPlayedBy(games, t.id()),
                "Each team should play exactly 8 games");
        }
    }
    ...
    void fourTeamsFillHomeAwayBalanced() {
        // Without balance tracking, some teams could end up always home in fill rounds (imbalance ≫ N-1).
        ...
        for (Team t : List.of(a, b, c, d)) {
            ...
            long away = games.stream().filter(g -> g.awayTeamId().equals(t.id())).count();
            assertTrue(Math.abs(home - away) <= n - 1,
                "Team " + t.name() + " home/away imbalance > " + (n - 1) + " after fill: home=" + home + " away=" + away);
        }
    }
    ...
    void threeTeamsFillAllTeamsReachAtLeastSevenGames() {
        ...
        for (Team t : List.of(a, b, c)) {
            ...
            assertTrue(count >= 7,
                "Team " + t.name() + " should have at least 7 games; got " + count);
        }
    }
    ...
    void noFillRoundLogsWhenTargetEqualsNMinus1() {
        ...
        assertInstanceOf(TeamScheduleResult.Success.class, result);
        assertTrue(((TeamScheduleResult.Success) result).fillRoundLogs().isEmpty(),
    ...
    void fillRoundLogsHaveCorrectCountAndFormat() {
        ...
        for (int i = 0; i < logs.size(); i++) {
            ...
            assertTrue(log.startsWith("Fill round " + (i + 1) + " complete:"),
                "Log entry " + i + " should start with 'Fill round " + (i + 1) + " complete:'; got: " + log);
    ...
    void twoDivisionsProduceCorrectGameCountsIndependently() {
        ...
        Division majors = division("Majors", 3,
            team("M1"), team("M2"), team("M3"), team("M4"));
        // AAA: 3 teams, target=2 → 3 games (N*(N-1)/2, no fill)
        Division aaa = division("AAA", 2,
    ...
    void gameNumbersAreSequentialFromOne() {
        ...
        List<Integer> numbers = games.stream()
            .map(TeamGame::gameNumber)
            .sorted()
            .collect(Collectors.toList());

        for (int i = 0; i < numbers.size(); i++) {
            assertEquals(i + 1, numbers.get(i),
                "Game number at sorted position " + i + " should be " + (i + 1));
        }
    }
