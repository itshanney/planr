...
@Command(
    ...
    subcommands = {
      ScheduleGameCommand.OverrideCmd.class,
      ScheduleGameCommand.EditHomeAwayCmd.class
    },
...
public class ScheduleGameCommand implements Runnable {
    ...
    public Integer call() {
      ...
      try {
        ...
        if (zeroIndex < 0 || zeroIndex >= schedule.games().size()) {
          System.err.printf(
              "Error: Game #%d not found (1–%d are valid).%n", gameNumber, schedule.games().size());
          return 1;
        }
        ...
        if (newHomeName != null) {
          ...
          if (team.isEmpty()) {
            System.err.printf(
                "Error: Team \"%s\" not found. Specify the team name as it appears in 'planr"
                    + " schedule view'.%n",
                newHomeName);
            return 1;
          }
  ...
  @Command(name = "edit", description = "Swap home/away for a game in the team schedule or draft.")
    ...
    public Integer call() {
      try {
        ...
        if (!isCurrentHome && !isCurrentAway) {
          System.err.printf(
              "Error: Team \"%s\" is not playing in game #%d " + "(home: %s, away: %s).%n",
