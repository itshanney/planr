...
public class PlayoffCommand implements Runnable {
  ...
  @Command(name = "generate", description = "Generate a double elimination bracket for a division.")
  static class GenerateCmd implements Callable<Integer> {
    ...
    @Option(
        ...
        paramLabel = "<team>",
        description =
            "Team name in seed order. Repeat once per team: "
    ...
    public Integer call() {
      ...
      try {
        ...
        if (!duplicates.isEmpty()) {
          System.err.printf(
              "Error: Duplicate team names in --seeds: %s.%n",
        ...
        List<String> unrecognized =
            seeds.stream()
                .filter(s -> division.teams().stream().noneMatch(t -> t.name().equalsIgnoreCase(s)))
                .toList();
        if (!unrecognized.isEmpty()) {
          System.err.printf(
              "Error: Unrecognized team name(s) in --seeds: %s.%n",
        ...
        List<PlayoffBracketService.BracketSlot> slots =
            bracketService.generateBracket(normalizedSeeds);

        List<PlayoffGame> games = slots.stream().map(PlayoffBracketService::toPlayoffGame).toList();
        ...
        System.out.printf(
            "%nPlayoff generated for %s: %d teams, %d game slots, %d bye(s).%n",
    ...
    private void printBracketSummary(List<PlayoffBracketService.BracketSlot> slots) {
      ...
      int roundW =
          Math.max(
              "ROUND".length(), slots.stream().mapToInt(s -> s.round().length()).max().orElse(0));
      int posAW =
          Math.max(
              "POSITION A".length(),
      ...
      String fmt =
          "%-" + slotW + "s  %-" + roundW + "s  %-" + posAW + "s  %-" + posBW + "s  %-" + byeW
              + "s%n";
      ...
      for (int i = 0; i < slots.size(); i++) {
        ...
        System.out.printf(
            fmt, "G" + (i + 1), s.round(), s.positionA(), s.positionB(), s.isBye() ? "YES" : "");
      }
    }
  }
  ...
  static class AssignCmd implements Callable<Integer> {
    ...
    public Integer call() {
      try {
        ...
        for (Playoff p : playoffs) {
          List<PlayoffGame> clearedGames =
              p.games().stream().map(PlayoffGame::withAssignmentCleared).toList();
        ...
        System.out.printf(
            "Playoff field assignment: %d game slots across %d division(s) " + "(%s to %s).%n",
        ...
        System.out.printf(
            "[0:00] Playoff field assignment started. " + "%d game slots across %d division(s).%n",
            totalRealGames, playoffs.size());
        System.out.flush();
        ...
        for (Playoff p : cleared.playoffs()) {
          ...
          Playoff updatedPlayoff =
              new Playoff(
                  p.divisionId(), p.startDate(), p.endDate(), PlayoffState.ASSIGNED, updatedGames);
  ...
  static class StatusCmd implements Callable<Integer> {
        ...
        if (divisionName == null) {
          ...
          for (Division div :
              allDivisions.stream()
                  ...
                  .toList()) {
            String state =
                league.findPlayoff(div.id()).map(p -> p.state().name()).orElse("NOT_STARTED");
    ...
    private void printBracketDetail(List<PlayoffGame> games, League league) {
      ...
      int roundW =
          Math.max(
              "ROUND".length(), games.stream().mapToInt(g -> g.round().length()).max().orElse(0));
      int posAW =
          Math.max(
              "POSITION A".length(),
      ...
      String fmt =
          "%-" + slotW + "s  %-" + roundW + "s  %-" + posAW + "s  %-" + posBW + "s  %-" + assignedW
              + "s%n";
      ...
      for (int i = 0; i < games.size(); i++) {
        ...
        if (g.isBye()) {
          ...
          assigned = String.format("%s %s %s", g.assignedDate(), g.assignedStartTime(), fieldName);
        } else {
          assigned = "UNASSIGNED";
        }
  ...
  static class ClearCmd implements Callable<Integer> {
    ...
    public Integer call() {
      try {
        ...
        Optional<Playoff> playoffOpt = league.findPlayoff(division.id());
        if (playoffOpt.isEmpty()) {
          System.err.printf("Error: No playoff exists for \"%s\".%n", division.name());
          return 1;
        }
        ...
        long assigned =
            playoff.games().stream().filter(g -> !g.isBye() && g.assignedDate() != null).count();
